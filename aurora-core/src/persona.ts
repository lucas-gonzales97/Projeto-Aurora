// Persona e montagem de prompt da Aurora — FONTE ÚNICA DE VERDADE (ADR-0016).
//
// Antes vivia em aurora-desktop/src/renderer/prompt.ts e foi COPIADA para
// aurora-mobile/server.mjs (duplicação real que este módulo elimina). Qualquer
// client — desktop, mobile, futura extensão — importa daqui; ninguém redefine
// a persona por conta própria (mesmo princípio do ADR-0004 para identidade
// visual, agora para a identidade cognitiva).
//
// Invariante central (ADR-0009 — Aurora crua): a base estática carrega APENAS
// persona, valores e limites. NENHUM dado do usuário mora aqui; tudo o que ela
// sabe do usuário entra em runtime via get_context (bloco CONTEXTO RECUPERADO
// DO VAULT). Vault vazio = ela não sabe nada ainda, e diz isso honestamente.
...
- Sua memória de longo prazo vem do vault via noesis-mcp (get_context), não de treino. No começo o vault está vazio — você conhece o usuário conversando, primeiro no onboarding e depois nas interações do dia a dia. Isso é esperado; não finja lembrar do que ainda não foi dito.
...
// Consciência temporal — injetada em runtime a cada envio. O modelo por si só
// não sabe "que horas são agora". Fuso fixo em America/Sao_Paulo (v0, uso BR).
...
// O que o chat manda como system: temporal + persona + (opcional) contexto do
// vault. Entities vazio/ausente = nenhum bloco de contexto — a única fonte de
// "memória do usuário" é o retrieval real.
